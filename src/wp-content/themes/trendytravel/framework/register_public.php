...
function dt_theme_enqueue_scripts() {
	...
	if(is_page_template('tpl-hotels.php') || is_singular('dt_places') || is_tax('hotel_entries') || is_page_template('tpl-hotels-search.php')):
		wp_enqueue_script('jq.colorbox', IAMD_FW_URL.'js/public/jquery.colorbox-min.js', array(), false, true);
	endif;
	
	wp_enqueue_script('jq.fancybox', IAMD_FW_URL.'js/public/jquery.fancybox.pack.js', array(), false, true);
	...
	wp_enqueue_script('jq.tooltipster', IAMD_FW_URL.'js/public/jquery.tooltipster.min.js', array(), false, true);
	...
	if(is_singular('dt_hotels') || is_singular('dt_places')):
		...
		wp_enqueue_script('jq.googlemap', IAMD_FW_URL.'js/public/jquery.gmap.min.js', array(), false, true);
	endif;
    	...
    	echo '<div class="banner">';
			...
			if( array_key_exists("show_search_section",$tpl_default_settings) ):
				if( array_key_exists("search_section_on_slider", $tpl_default_settings) ):
					...
					require_once get_template_directory()."/includes/hotels-searchform.php";
					echo '</div>';
				endif;
			endif;			
		echo '</div>';
