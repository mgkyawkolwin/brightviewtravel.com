...
$str += '	<a href="" title="" class="style-picker-ico"> <img src="' + mytheme_urls.theme_base_url + 'images/style-picker/picker-icon.png" alt="close" /> </a>';
...
jQuery(document).ready(function ($) {
    ...
    if ($.cookie("traveltheme_skin") !== null) {
        var $href = $("link[id='skin-css']").attr("href");
        $href = $href.substr(0, $href.lastIndexOf("/"));
        $href = $href.substr(0, $href.lastIndexOf("/")) + "/" + $.cookie("traveltheme_skin") + "/style.css";
        $("link[id='skin-css']").attr("href", $href);
        $("ul.color-picker a[id='" + $.cookie("traveltheme_skin") + "']").addClass("selected");
		...
		$(".gallery-container img.item-mask, .gallery-carousel-wrapper img.item-mask, .dt-sc-team img.item-mask").each(function(){
			...
			$(this).attr("src",mytheme_urls.theme_base_url+"skins/"+$.cookie("traveltheme_skin")+"/images/"+data);
		});
    ...
    //Apply Layout
    if ($.cookie("traveltheme_layout") == "boxed") {
        $("ul.layout-picker li a").removeAttr("class");
        $("ul.layout-picker li a[id='" + $.cookie("traveltheme_layout") + "']").addClass("selected");
        $("div#pattern-holder").removeAttr("style");

        var $i = ($.cookie("traveltheme_pattern")) ? $.cookie("traveltheme_pattern") : 'pattern1';
        $img = mytheme_urls.theme_base_url + "images/patterns/" + $i + ".jpg";
        $('body').css('background-image', 'url(' + $img + ')').addClass('boxed');
        $("ul.pattern-picker a[id=" + $.cookie("traveltheme_pattern") + "]").addClass('selected');
    }
    ...
    $("ul.layout-picker a").click(function (e) {
        ...
        $.cookie("traveltheme_layout", $this.attr("id"), { path: '/' });

        if ($.cookie("traveltheme_layout") === "boxed") {
            $("body").addClass("boxed");
            $("div#pattern-holder").slideDown();

            if ($.cookie("traveltheme_pattern") === null) {
                $("ul.pattern-picker a:first").addClass('selected');
                $.cookie("traveltheme_pattern", "pattern1", {
                    path: '/'
                });
            } else {
                $("ul.pattern-picker a[id=" + $.cookie("traveltheme_pattern") + "]").addClass('selected');
                $img = mytheme_urls.theme_base_url + "images/patterns/" + $.cookie("traveltheme_pattern") + ".jpg";
    ...
    $("ul.pattern-picker a").click(function (e) {
        if ($.cookie("traveltheme_layout") == "boxed") {
            ...
            $("ul.pattern-picker a").removeAttr("class");
            $this.addClass("selected");
            $.cookie("traveltheme_pattern", $this.attr("id"), {
                path: '/'
            });
            $img = mytheme_urls.theme_base_url + "images/patterns/" + $.cookie("traveltheme_pattern") + ".jpg";
            $('body').css('background-image', 'url(' + $img + ')');
    ...
    $("ul.color-picker a").click(function (e) {
        ...
        $this.addClass("selected");
        $.cookie("traveltheme_skin", $this.attr("id"), {
            path: '/'
        });
