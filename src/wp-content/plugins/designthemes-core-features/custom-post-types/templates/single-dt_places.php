<?php get_header();
	...
	while(have_posts()): the_post();
	  ...
	  $map_code = '<div class="widget">';
	  	$map_code .= '<h3 class="widgettitle">'.__('Map of ', 'dt_themes').get_the_title().'</h3>';
		$map_code .= '<div id="place_map_'.get_the_ID().'" class="list-hotel-map" data-add="'.get_the_title().', '.@$meta_set['place_add'].'" data-lt="'.@$meta_set['place_lat'].'" data-lg="'.@$meta_set['place_long'].'"></div>';
	  $map_code .= '</div>'; ?>
          ...
          <div class="container">
            ...
            <?php endif; ?>
                ...
                <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<h1 class="section-title place-heading"><?php the_title(); ?></h1><sub><?php echo @$meta_set['place_add'];?></sub><?php
										...
										<ul><?php
											foreach($hotel_array as $hid):
                                                ...
                                                <li>
                                                    ...
                                                    <h6><a href="<?php echo get_permalink($hid); ?>"><?php echo $tpost->post_title; ?></a>, <sub><?php echo @$hmeta['hotel_add']; ?></sub></h6>
                                        ...
                                        <ul><?php
											foreach($dests_array as $did):
                                                ...
                                                <li>
                                                    ...
                                                    <h6><a href="<?php echo get_permalink($did); ?>"><?php echo $tpost->post_title; ?></a>, <sub><?php echo @$pmeta['place_add']; ?></sub></h6>
                                                    <?php echo '<div class="star-rating-wrapper"><div class="star-rating"><span style="width:'.(($all_avg/5)*100).'%"></span></div>('.count($arr_rate).__(' Ratings', 'dt_themes').')</div>'; ?>
					...
					if(isset($meta_set['show-recommends']) != ""): ?>
						...
						if($the_query->have_posts()):
                          ...
                          <div class="carousel_items">
                            <div class="dt-sc-places-wrapper dt_carousel" data-items="<?php echo $maxitems; ?>"><?php
